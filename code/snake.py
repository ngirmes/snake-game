...
from os import walk # let's the user travel through a folder
...
class Snake:
    ...
    # Retrieves the images for the snake body
    def import_surfs(self):
        surf_dict = {}
        for folder_path, _, image_names in walk(join('..', 'graphics', 'snake')):
            print(folder_path, image_names)
            for image_name in image_names:
                full_path = join(folder_path, image_name)
                surface = pygame.image.load(full_path).convert_alpha() # convert_alpha to convert data so pygame can run faster
